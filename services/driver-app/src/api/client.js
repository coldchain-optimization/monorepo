...
function resolveApiBase() {
  ...
  const normalizedHost =
    host === "127.0.0.1" && Platform.OS === "android" ? "10.0.2.2" : host;
...
class ApiClient {
  ...
  async updateLocation(shipmentId, payload) {
    return this.request(
      "POST",
      `/tracking/${encodeURIComponent(shipmentId)}/location`,
      payload,
    );
  }

  // ML Endpoints
  async findBackhaul(payload) {
    return this.request("POST", "/ml/find_backhaul", payload);
  }

  async optimizeRoute(payload) {
    return this.request("POST", "/ml/optimize_route", payload);
  }
}
