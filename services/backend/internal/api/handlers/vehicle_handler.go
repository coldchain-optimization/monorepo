...
func (vh *VehicleHandler) UpdateVehicle(c *gin.Context) {
	...
	if v, ok := req["vehicle_type"].(string); ok {
		vehicle.VehicleType = v
	}
	if v, ok := req["license_plate"].(string); ok {
		vehicle.LicensePlate = v
	}
	if v, ok := req["manufacturer"].(string); ok {
		vehicle.Manufacturer = v
	}
	if v, ok := req["model"].(string); ok {
		vehicle.Model = v
	}
	if v, ok := req["fuel_type"].(string); ok {
		vehicle.FuelType = v
	}
	if v, ok := req["current_location"].(string); ok {
		vehicle.CurrentLocation = v
	}
	if v, ok := req["is_available"].(bool); ok {
		vehicle.IsAvailable = v
	}
	if v, ok := req["is_refrigerated"].(bool); ok {
		vehicle.IsRefrigerated = v
	}
	if v, ok := req["capacity"].(float64); ok {
		vehicle.Capacity = int(v)
	}
	if v, ok := req["max_weight"].(float64); ok {
		vehicle.MaxWeight = int(v)
	}
	if v, ok := req["temperature"].(float64); ok {
		vehicle.Temperature = int(v)
	}
	if v, ok := req["year"].(float64); ok {
		vehicle.Year = int(v)
	}
	if v, ok := req["carbon_footprint"].(float64); ok {
		vehicle.CarbonFootprint = v
	}
